...
extern wiced_result_t BTM_SetDataChannelPDULength(wiced_bt_device_address_t bd_addr, 
                                                  uint16_t tx_pdu_length);


#ifdef ENABLE_OTA
static wiced_bt_gatt_status_t 
app_bt_prepare_write_handler(uint16_t conn_id,
                                                           wiced_bt_gatt_opcode_t opcode,
                                                           wiced_bt_gatt_write_req_t *p_req,
                                                           uint16_t *p_error_handle);

static wiced_bt_gatt_status_t 
app_bt_execute_write_handler(wiced_bt_gatt_event_data_t *p_req, 
                                                           uint16_t *p_error_handle);
                         ...
                         wiced_bt_gatt_event_data_t *p_event_data)
{
    ...
    switch (event)
    {
        ...
        case GATT_CONNECTION_STATUS_EVT:
            printf("gatt connection status evt \r\n");
...
app_bt_gatt_connection_status_change_cb(wiced_bt_gatt_connection_status_t *p_conn_status)
{
    ...
    if ((p_conn_status->connected) && (0 == app_bt_conn_id))
    {
        ...
        app_bt_adv_stop();
        if(xTimerIsTimerActive(adv_stop_timer))
        {
        ...
        /* Enable light_sleep for motion sensor for better response in connected state */
        app_motion_sleep(MOTION_LIGHT_SLEEP);

        /* Enable scroll when connected */
        app_scroll_enable();

        wiced_bt_l2cap_update_ble_conn_params(peer_bd_addr,
                                              MIN_CI,
                                              MAX_CI,
                                              SLAVE_LATENCY,
                                              SUPERVISION_TO);
    ...
    else
    {
        ...
        if(motion_activity_stat)
        {
            ...
            {
                printf("Failed to start Motion data Timer\r\n");
            }
        }
        ...
        /* Disable Scroll in disconnected state */
        app_scroll_disable();
#endif
...
#endif
            break;/* Value confirmation */
            break;

        case GATT_HANDLE_VALUE_NOTIF:
            /* If motion event is present, continue processing it */
            if(motion_activity_stat)
            {
                if (pdFAIL == xTimerStart(motion_data_read_timer, TIMER_MAX_WAIT))
                {
    ...
    *p_error_handle = p_write_req->handle;
    ...
    index = app_bt_gatt_get_index_by_handle(p_write_req->handle);
           ...
           case HDLC_OTA_FW_UPGRADE_SERVICE_OTA_UPGRADE_CONTROL_POINT_VALUE:
               ...
               case CY_OTA_UPGRADE_COMMAND_VERIFY:
                   ...
                   if (result == CY_RSLT_SUCCESS)
                   {
                        ...
                        uint8_t bt_notify_buff = CY_OTA_UPGRADE_STATUS_OK;
                        gatt_status = app_bt_ble_send_indication(app_bt_conn_id, HDLC_OTA_FW_UPGRADE_SERVICE_OTA_UPGRADE_CONTROL_POINT_VALUE, 1, &bt_notify_buff);
                        if (gatt_status != WICED_BT_GATT_SUCCESS)
                        {
                   ...
                   else
                   {
                       ...
                       gatt_status = app_bt_ble_send_indication(app_bt_conn_id,
                                                                HDLC_OTA_FW_UPGRADE_SERVICE_OTA_UPGRADE_CONTROL_POINT_VALUE,
                                                                1, 
                                                                &bt_notify_buff);
           ...
           case HDLC_OTA_FW_UPGRADE_SERVICE_OTA_UPGRADE_DATA_VALUE:
               printf("Application downloading in progress... \r\n");
               result = cy_ota_ble_download_write(ota_app.ota_context,
                                                  p_write_req->p_val,
                                                  p_write_req->val_len,
                                                  p_write_req->offset);
               if (result != CY_RSLT_SUCCESS)
...
/**
 * Function Name:
 * app_bt_gatt_get_attr_value
 *
 * Function Description:
 * @brief  The function is invoked by app_bt_gatt_attr_read_handler to get a Value from
 *         GATT DB.
 *
 ...
 */
wiced_bt_gatt_status_t
app_bt_gatt_get_attr_value(uint8_t attr_handle_index,
                     uint8_t *p_val,
                     uint16_t len,
                     uint16_t *p_len)
{
    ...
    else
    {
        ...
        *p_len = app_gatt_db_ext_attr_tbl[attr_handle_index].cur_len;
        memcpy(p_val, app_gatt_db_ext_attr_tbl[attr_handle_index].p_data, len);
        ...
        if (NULL != memcpy(app_gatt_db_ext_attr_tbl[attr_handle_index].p_data, p_val, len))
        {
            ...
            switch (app_gatt_db_ext_attr_tbl[attr_handle_index].handle)
            {
                case HDLD_BAS_BATTERY_LEVEL_CLIENT_CHAR_CONFIG:
                    ...
                    break;
                case HDLD_HIDS_MOUSE_IN_REPORT_CLIENT_CHAR_CONFIG:
                    printf("Mouse IN report ");
                    app_bt_bond_modify_cccd_in_nv_storage(
                            app_gatt_db_ext_attr_tbl[attr_handle_index].handle, p_val);
                    break;
                case HDLD_HIDS_BOOT_MOUSE_INPUT_REPORT_CLIENT_CHAR_CONFIG:
                    printf("Boot Mouse IN report ");
                    app_bt_bond_modify_cccd_in_nv_storage(
                            app_gatt_db_ext_attr_tbl[attr_handle_index].handle, p_val);
                    break;
...
/**
 * Function Name:
 * app_bt_gatt_enable_all_cccds
 *
 * Function Description:
 * @brief This function is used to enable all CCCD
 *
 * @param void
 *
 * @return void
 */
void app_bt_gatt_enable_all_cccds(void)
{
    app_bas_battery_level_client_char_config[0] = 0x01;
    app_hids_mouse_in_report_client_char_config[0] = 0x01;
    app_hids_boot_mouse_input_report_client_char_config[0] = 0x01;
...
void app_bt_gatt_disable_all_cccds(void)
{
    ...
    app_hids_mouse_in_report_client_char_config[0] = 0x00;
    app_hids_boot_mouse_input_report_client_char_config[0] = 0x00;
...
static wiced_bt_gatt_status_t app_bt_execute_write_handler(wiced_bt_gatt_event_data_t *p_req, uint16_t *p_error_handle)
{
    ...
    *p_error_handle = p_write_req->handle;
    ...
    if(write_buff.in_use == false)
    {
        printf("write_buff.inuse is false returning error...\n");
        return WICED_BT_GATT_ERROR;
    }

    printf("Execute Write with %d bytes\n", write_buff.written);
